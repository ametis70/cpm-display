...
local AUTHOR_NOTE =
    "-[ JSON.lua package by Jeffrey Friedl (http://regex.info/blog/lua/json) version 20211016.28 ]-"
...
--             if type(etc) = 'table' and etc.photo thenjson
...
local isArray = {
    __tostring = function() return "JSON array" end
}
isArray.__index = isArray
local isObject = {
    __tostring = function() return "JSON object" end
}
...
function OBJDEF:asNumber(item)
    ...
    if getmetatable(item) == isNumber then
        ...
        return item
    elseif type(item) == 'table' and type(item.S) == 'string' and type(item.N) ==
        'number' then
...
--
local function top_three_bits(val) return math.floor(val / 0x20) end

local function top_four_bits(val) return math.floor(val / 0x10) end

local function unicode_character_bytecount_based_on_first_byte(first_byte)
    local W = string.byte(first_byte)
    if W < 0x80 then
        return 1
    elseif (W == 0xC0) or (W == 0xC1) or (W >= 0x80 and W <= 0xBF) or
        (W >= 0xF5) then
...
local function unicode_codepoint_as_utf8(codepoint)
    ...
    elseif codepoint <= 65535 then
        ...
        return string.char(0xF0 + highpart, 0x80 + midA, 0x80 + midB,
                           0x80 + lowpart)
    end
end
...
local function grok_number(self, text, start, options)
    ...
    --
    local integer_part = text:match('^-?[1-9]%d*', start) or
                             text:match("^-?0", start)
    ...
    if options.decodeNumbersAsObjects then
        ...
        if not options.decodeIntegerObjectificationLength and
            not options.decodeDecimalObjectificationLength then
            -- no options, so objectify
            objectify = true

        elseif (options.decodeIntegerObjectificationLength and
            (integer_part:len() >= options.decodeIntegerObjectificationLength or
                exponent_part:len() > 0)) or
            (options.decodeDecimalObjectificationLength and
                (decimal_part:len() >=
                    options.decodeDecimalObjectificationLength or
                    exponent_part:len() > 0)) then
            -- have options and they are triggered, so objectify
            objectify = true
        end

        if objectify then return OBJDEF:asNumber(full_number_text), i end
        ...
        --
        if (options.decodeIntegerStringificationLength and
            (integer_part:len() >= options.decodeIntegerStringificationLength or
                exponent_part:len() > 0)) or

            (options.decodeDecimalStringificationLength and
                (decimal_part:len() >=
                    options.decodeDecimalStringificationLength or
                    exponent_part:len() > 0)) then
...
local function grok_string(self, text, start, options)
    ...
    while i <= text_len do
        ...
        if c ~= '\\' then
            ...
            local byte_count =
                unicode_character_bytecount_based_on_first_byte(c)
            ...
            elseif byte_count == 1 then
                if options.strictParsing and string.byte(c) < 0x20 then
                    self:onDecodeError("Unescaped control character", text,
                                       i + 1, options.etc)
                    return nil, start -- in case the error method doesn't abort, return something sensible
                end
                next_character = c
            ...
            elseif byte_count == 4 then
                next_character = text:match(
                                     '^(.[\128-\191][\128-\191][\128-\191])', i)
            end

            if not next_character then
                self:onDecodeError("incomplete utf8 sequence", text, i,
                                   options.etc)
            ...
            if backslash_escape_conversion[next_byte] then
                ...
                elseif options.strictParsing then
                    ...
                    self:onDecodeError("illegal use of backslash escape", text,
                                       i, options.etc)
                    return nil, start -- in case the error method doesn't abort, return something sensible
                else
                    local byte_count =
                        unicode_character_bytecount_based_on_first_byte(
                            next_byte)
                    if byte_count == 0 then
                        self:onDecodeError(
                            "non-utf8 sequence after backslash escape", text, i,
                            options.etc)
                        return nil, start -- in case the error method doesn't abort, return something sensible
                    end
                    ...
                    if next_character == nil then
                        ...
                        self:onDecodeError(
                            "incomplete utf8 sequence after backslash escape",
                            text, i, options.etc)
                        return nil, start -- in case the error method doesn't abort, return something sensible
                    end
...
function OBJDEF:decode(text, etc, options)
    ...
    --
    if etc ~= nil then options.etc = etc end
    ...
    elseif type(text) ~= 'string' then
        ...
        self:onDecodeError(
            string.format("%s, got %s", error_message, type(text)), nil, nil,
            options.etc)
        return nil, error_message -- in case the error method doesn't abort, return something sensible
    end
    ...
    --
    if text:sub(1, 1):byte() == 0 or
        (text:len() >= 2 and text:sub(2, 2):byte() == 0) then
    ...
    if success then
        ...
        if next_i ~= #text + 1 then
            ...
            if next_i ~= #text + 1 then
                value, error_message = self:onTrailingGarbage(text, next_i,
                                                              value, options.etc)
            end
        end
...
local function json_string_literal(value, options)
    ...
    if options.stringsAreUtf8 then
        ...
        --
        newval = newval:gsub(LINE_SEPARATOR_as_utf8, '\\u2028'):gsub(
                     PARAGRAPH_SEPARATOR_as_utf8, '\\u2029')
    end
    return '"' .. newval .. '"'
end
...
local function object_or_array(self, T, etc)
    ...
    for key in pairs(T) do
        ...
        elseif type(key) == 'boolean' then
            ...
            self:onEncodeError("can't encode table with a key of type " ..
                                   type(key), etc)
        end
    end
...
local function encode_value(self, value, parents, etc, options, indent, for_key)
    ...
    --
    if value == nil or
        (not for_key and options and options.null and value == options.null) then
        return 'null'
    ...
    elseif type(value) ~= 'table' then
        ...
        if self.unsupportedTypeEncoder then
            local user_value, user_error =
                self:unsupportedTypeEncoder(value, parents, etc, options,
                                            indent, for_key)
            ...
            elseif user_value ~= nil then
                self:onEncodeError(
                    "unsupportedTypeEncoder method returned a " ..
                        type(user_value), etc)
    ...
    elseif getmetatable(value) == isNumber then
        ...
        if parents[T] then
            self:onEncodeError("table " .. tostring(T) ..
                                   " is a child of itself", etc)
        else
            parents[T] = true
        end
        ...
        elseif object_keys then
            ...
            if options.pretty then
                ...
                for _, key in ipairs(object_keys) do
                    local encoded = encode_value(self, tostring(key), parents,
                                                 etc, options, indent, true)
                ...
                local FORMAT = "%s%" .. string.format("%d", max_key_length) ..
                                   "s: %s"
                ...
                for i, key in ipairs(object_keys) do
                    local encoded_val = encode_value(self, TT[key], parents,
                                                     etc, options,
                                                     subtable_indent)
                    table.insert(COMBINED_PARTS, string.format(FORMAT,
                                                               key_indent,
                                                               KEYS[i],
                                                               encoded_val))
                end
                ...
                for _, key in ipairs(object_keys) do
                    ...
                    local encoded_key = encode_value(self, tostring(key),
                                                     parents, etc, options,
                                                     indent, true)
                    table.insert(PARTS, string.format("%s:%s", encoded_key,
                                                      encoded_val))
                end
...
function OBJDEF:encode_pretty(value, etc, options)
    if type(self) ~= 'table' or self.__index ~= OBJDEF then
        OBJDEF:onEncodeError(
            "JSON:encode_pretty must be called in method format", etc)
    end

    --
    -- If the user didn't pass in a table of decode options, use the default pretty ones
    --
    if type(options) ~= 'table' then options = default_pretty_options end
